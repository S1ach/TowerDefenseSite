...
export function placementError(p: Position, occupied: Position[]): string | null {
  ...
  if (Math.abs(p.x) > battlefield.width / 2 - 0.8 || Math.abs(p.z) > battlefield.depth / 2 - 0.8) return 'За границей поля';
  if (distanceToPath(p) < battlefield.roadWidth / 2 + 0.7) return 'Нельзя строить на дороге';
  if (occupied.some(t => Math.hypot(t.x - p.x, t.z - p.z) < 1.6)) return 'Слишком близко к другой башне';
  return null;
}
